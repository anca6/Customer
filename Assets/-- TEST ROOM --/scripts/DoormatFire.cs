...
public class DoormatFire : MonoBehaviour
{
    ...
    private void OnCollisionStay(Collision collision)
    {
        if (collision.gameObject.CompareTag("Player"))
        {
            ...
            if (Input.GetKey(ActivationKey)) // if the player presses the activation key
            {
                ...
                {
                    if (Burning && !isLocked) // pretty self explanatory
                    {
                        Burning = false; // the door is open
                        Debug.Log("fire out");
                    ...
                    {
                        Burning = true; // the door is closed
                        Debug.Log("Closed");
            ...
            if (playerInventory != null) // if the player has picked up objects in the inventory
            {
                if (Input.GetKey(LockKey)) // if they want to lock the door
                {
                    ...
                    {
                        if (Burning && !isLocked && playerInventory.inventory.Contains(requiredPickupTag)) // if the door is closed and they have the key in inventory
                        {
                            isLocked = true; // door is locked
                            Debug.Log("Locked!");
                            lockScore = true; // for reward system
                        }
                        else if (Burning && isLocked && playerInventory.inventory.Contains(requiredPickupTag)) // if the door is closed, locked and they have the key
                        {
